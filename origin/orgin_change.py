...
MIN_DETECTION_AREA = 50 # 最小检测面积，过滤小噪点
...
Ki = 0.001 # Integral gain 0.01
Kd = 0.6  # Derivative gain 0.1
...
class PIDControllerx:
    def __init__(self, Kp, Ki, Kd, integral_limit=50, output_limit=100,deadzone=8.0):
        self.Kp = Kp
        self.Ki = Ki
        self.Kd = Kd
        self.last_error = 0
...
def detection():
    ...
    try:
        ...
        #初始化PID控制器
        pid_x = PIDControllerx(Kp-0.621, Ki+0.1, Kd-0.45,integral_limit=30, output_limit=30,deadzone=1.0)
        ...
        while  True:
            ...
            with ScopedTiming("total",debug_mode > 0):
                ...
                if rgb888p_img.format() == image.RGBP888:
                    ...
                    osd_img.clear()
                    # 在循环前定义原始图像中心点
                    algo_center_x = OUT_RGB888P_WIDTH // 2  # 540
                    algo_center_y = OUT_RGB888P_HEIGH // 2  # 360
                    ...
                    if det_boxes:
                        for det_boxe in det_boxes:
                            ...
                            Servo_data[0] = int(round(output_x))  # 四舍五入转换为整数
                            Servo_data[1] = int(round(servo_dy))
                            Servo_data[2] = distance
                            send_packet(Servo_data)


                            # 显示相关的坐标转换（仅用于绘制，不影响控制）
                            center_x_display = int(center_x * DISPLAY_WIDTH / OUT_RGB888P_WIDTH)
                            center_y_display = int(center_y * DISPLAY_HEIGHT / OUT_RGB888P_HEIGH)
                            ...
                            osd_img.draw_rectangle(int(x1 * DISPLAY_WIDTH / OUT_RGB888P_WIDTH),
                                                  ...
                                                  int(w), int(h), color=color_four[det_boxe[0]][1:])

                            # 添加距离显示
                            osd_img.draw_string(center_x_display - 30, center_y_display - 20,
